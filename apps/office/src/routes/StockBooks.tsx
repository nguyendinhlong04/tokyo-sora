    ...
    <>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-8 pb-8">
        ...
        <div className="mt-5 overflow-hidden rounded-md border border-line-1 bg-surface-1">
          <div className="grid grid-cols-[1fr_130px_110px_120px_130px_130px_110px] gap-3 border-b border-line-1 bg-canvas px-5 py-3 text-[length:var(--fs-c2)] font-semibold tracking-[0.1em] text-ink-mute uppercase">
          ...
          {book.isPending ? (
            ...
            <p className="px-5 py-4 text-[length:var(--fs-b2)] text-ink-mute">
              Chưa có lô nào. Lô sinh ra khi nhập hàng bắt buộc khai lô ở màn Nhập kho.
            </p>
          ) : (
            lots.map((lot) => (
              <LotLine
...
function LotLine({
  ...
  const band = BANDS[lot.band]
  return (
    <div
      className={`grid grid-cols-[1fr_130px_110px_120px_130px_130px_110px] items-center gap-3 border-b border-line-1 px-5 py-2.5 last:border-b-0 ${
        lot.qtyRemainBase === 0 ? 'opacity-50' : ''
      }`}
    >
      ...
      <span className="font-mono text-[length:var(--fs-c1)] text-ink-mute">
        ...
        {lot.receiveTempDeciC !== null ? (
          <span className="mt-0.5 block">{(lot.receiveTempDeciC / 10).toFixed(1)}°C</span>
        ) : null}
      </span>
      <span className={`font-mono text-[length:var(--fs-c1)] ${band.className}`}>
        {lot.expiresOn ? formatDay(lot.expiresOn) : '—'}
        {lot.state === 'open' && lot.labelExpiresOn && lot.labelExpiresOn !== lot.expiresOn ? (
      ...
      <span className="flex flex-wrap items-center gap-1.5">
        ...
        <span className={`text-[length:var(--fs-c2)] ${band.className}`}>{band.label}</span>
      </span>
      <span className="flex justify-end">
        {lot.tappable && mayTap ? (
          <button
            ...
            disabled={busy}
            onClick={onTap}
            className="h-8 rounded-sm border border-accent px-2 text-[length:var(--fs-c1)] text-accent-ink hover:bg-surface-3"
          >
            Đục keg
          </button>
        ) : null}
...
function Tile({
  ...
  return (
    <div className={`rounded-md border ${tone ? `border-${tone}` : 'border-line-1'} bg-surface-1 px-5 py-4`}>
      <p className="text-[length:var(--fs-c2)] font-semibold tracking-[0.12em] text-ink-mute uppercase">
        {label}
      </p>
      <p className={`mt-2 font-mono text-[length:var(--fs-d3)] leading-none ${color}`}>{count} lô</p>
  ...
  return (
    <>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-8 pb-8">
          ...
          <section className="mt-5 rounded-md border border-line-1 bg-surface-1 p-5">
            ...
            <div className="mt-3 flex flex-col gap-2">
              {data.draftBeer.map((beer) => (
                ...
                >
                  <span className="text-[length:var(--fs-b2)] text-ink-hi">{beer.ingredientName}</span>
        ...
        <div className="mt-5 overflow-hidden rounded-md border border-line-1 bg-surface-1">
          <div className="grid grid-cols-[1fr_130px_130px_120px_120px_130px_110px] gap-3 border-b border-line-1 bg-canvas px-5 py-3 text-[length:var(--fs-c2)] font-semibold tracking-[0.1em] text-ink-mute uppercase">
            <span>Nguyên liệu</span>
            <span className="text-right">Công thức đòi</span>
            <span className="text-right">Thực tế xuất</span>
            <span className="text-right">Huỷ</span>
            <span className="text-right">Nội bộ</span>
          ...
          {report.isPending ? (
            ...
            <p className="px-5 py-4 text-[length:var(--fs-b2)] text-ink-mute">
              Chưa có tiêu hao nào trong khoảng này.
            </p>
          ) : (
            rows.map((row) => (
              <div
                key={row.ingredientId}
                className="grid grid-cols-[1fr_130px_130px_120px_120px_130px_110px] items-center gap-3 border-b border-line-1 px-5 py-2 last:border-b-0"
              >
                <span className="min-w-0 truncate text-[length:var(--fs-b2)] text-ink-hi">
                  {row.ingredientName}
                  <span className="ml-2 text-[length:var(--fs-c1)] text-ink-mute">{row.baseUnit}</span>
                </span>
                <span className="text-right font-mono text-[length:var(--fs-c1)] text-ink-mute">
                  {row.theoreticalBase}
                </span>
                ...
                <span className="text-right font-mono text-[length:var(--fs-c1)] text-warn">
                  {row.writeOffBase || '—'}
                </span>
                <span className="text-right font-mono text-[length:var(--fs-c1)] text-ink-mute">
                  {row.internalBase || '—'}
                ...
                <span
                  className={`text-right font-mono text-[length:var(--fs-c1)] ${
                    row.diffVnd > 0 ? 'text-danger' : row.diffVnd < 0 ? 'text-ok' : 'text-ink-mute'
                  }`}
                >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-8 pb-8">
        ...
        {ingredientId === '' ? (
          ...
          <>
            ...
            <div className="mt-5 overflow-hidden rounded-md border border-line-1 bg-surface-1">
              <div className="grid grid-cols-[110px_150px_1fr_110px_130px_130px] gap-3 border-b border-line-1 bg-canvas px-5 py-3 text-[length:var(--fs-c2)] font-semibold tracking-[0.1em] text-ink-mute uppercase">
                <span>Ngày</span>
                <span>Loại</span>
                <span>Chi tiết</span>
                <span className="text-right">Lượng</span>
                <span className="text-right">Tiền</span>
                <span className="text-right">Tồn sau</span>
              </div>
              ...
              {data.moves.length === 0 ? (
                ...
                data.moves.map((move) => (
                  <div
                    key={move.id}
                    className="grid grid-cols-[110px_150px_1fr_110px_130px_130px] items-center gap-3 border-b border-line-1 px-5 py-2 last:border-b-0"
                  >
                    <span className="font-mono text-[length:var(--fs-c1)] text-ink-mute">
                    ...
                    <span className="min-w-0 truncate text-[length:var(--fs-c1)] text-ink-mute">
                    ...
                    <span
                      className={`text-right font-mono text-[length:var(--fs-c1)] ${
                        move.qtyBase > 0 ? 'text-ok' : 'text-ink-body'
                      }`}
                    >
